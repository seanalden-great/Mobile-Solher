...
class Category {
  ...
  final DateTime? bundleStartDate;
  final DateTime? bundleEndDate;
  final Map<String, dynamic>? promoConfig; // Menyimpan raw JSON config
  final List<dynamic>? products;
  ...
  factory Category.fromJson(Map<String, dynamic> json) {
    // Menyesuaikan dengan struktur Controller yang baru
    final data = json.containsKey('data') ? json['data'] : json;

    // Data promo sekarang murni berada di dalam object 'promo_config'
    final promo = data['promo_config'];
    ...
    return Category(
      ...
      description:
          data['meta'] != null ? data['meta']['description']?.toString() : null,
      promoConfig: promo,
      bundleQty: promo != null && promo['qty'] != null
          ? (promo['qty'] is int
              ? promo['qty']
              : int.tryParse(promo['qty'].toString()))
          : null,
      bundlePrice: promo != null ? promo['price'] : null,
      bundleStartDate: promo != null && promo['start_date'] != null
          ? DateTime.tryParse(promo['start_date'].toString())
          : null,
      bundleEndDate: promo != null && promo['end_date'] != null
          ? DateTime.tryParse(promo['end_date'].toString())
          : null,
      products: data['products'],
    );
  }
  ...
  Map<String, dynamic> toJson() {
    // Memformat payload agar diterima mulus oleh validasi Laravel (code, name, description, promo_config)
    Map<String, dynamic>? promoPayload = promoConfig;

    // Fallback jika dikonstruksi secara manual dari Mobile
    if (promoPayload == null && (bundleQty != null || bundlePrice != null)) {
      promoPayload = {
        'promo_type': 'bundle',
        'qty': bundleQty,
        'price': bundlePrice,
        'start_date': bundleStartDate?.toIso8601String(),
        'end_date': bundleEndDate?.toIso8601String(),
      };
    }
