...
// // 👇 [BARU] Import BLoC Produk untuk memuat rekomendasi 👇
...
//     // 👇 MENGGUNAKAN BLOC PROVIDER UNTUK PRODUK REKOMENDASI 👇
//     return BlocProvider(
...
//                           bool hasDiscount = item.product!.hasActiveDiscount;
//                           num activePrice = hasDiscount
//                               ? item.product!.discountPrice!
//                               : item.product!.price;
...
//                         // 👇 [BARU] SECTION REKOMENDASI PRODUK 👇
...
//   // ===========================================================================
//   // 👇 WIDGET REKOMENDASI CROSS-SELLING 👇
//   // ===========================================================================
//   Widget _buildRecommendations() {
//     return BlocBuilder<ProductBloc, ProductState>(
//       builder: (context, state) {
//         if (state is ProductListLoaded) {
//           // Ambil ID produk yang sudah ada di keranjang untuk difilter
//           final cartProductIds = _currentItems.map((e) => e.productId).toSet();
          
//           // Cari produk yang belum ada di keranjang
//           var suggestions = state.products
//               .where((p) => !cartProductIds.contains(p.id) && p.stock > 0)
//               .toList();
          
//           if (suggestions.isEmpty) return const SizedBox.shrink();

//           // Acak urutan agar selalu segar setiap dibuka
...
//                       child: SizedBox(
//                         width: 130,
//                         child: _buildSuggestedProductCard(product),
//                       ),
//                     );
//                   },
//                 ),
...
//   Widget _buildSuggestedProductCard(ProductModel product) {
...
//           ),
//         ).then((_) {
//           // Memperbarui keranjang jika user menambahkan barang dari halaman detail
//           context.read<CartBloc>().add(FetchCartEvent());
//         });
//       },
//       child: Column(
...
// import 'package:solher_mobile/blocs/auth/auth_state.dart';
...
//   // 👇 FUNGSI BACA KERANJANG GUEST LOKAL 👇
//   Future<void> _loadGuestCart() async {
//     final prefs = await SharedPreferences.getInstance();
//     final String? cartStr = prefs.getString('guest_cart');

//     if (cartStr != null && cartStr.isNotEmpty) {
//       final List<dynamic> decoded = json.decode(cartStr);
//       List<CartModel> parsedItems = [];

//       for (int i = 0; i < decoded.length; i++) {
//         var itemData = decoded[i];
//         // Jika struktur dari Vue tidak memiliki ID, beri ID unik palsu untuk UI
//         itemData['id'] =
//             itemData['id'] ?? (DateTime.now().millisecondsSinceEpoch + i);
//         try {
//           parsedItems.add(CartModel.fromJson(itemData));
//         } catch (e) {
//           // Skip if error
//         }
//       }
...
//           // Otomatis pilih semua item layaknya di Vue
//           _selectedIds = _currentItems.map((e) => e.id).toList();
//           _isAllSelected = true;
//         });
//       }
//     }
...
//   // 👇 FUNGSI SIMPAN KERANJANG GUEST LOKAL 👇
//   Future<void> _saveGuestCart() async {
//     final prefs = await SharedPreferences.getInstance();
//     final listMap = _currentItems
//         .map((e) => {
//               'id': e.id,
//               'product_id': e.product?.id,
//               'quantity': _optimisticQty[e.id] ?? e.quantity,
//               'color': e.color,
//               'product':
//                   e.product?.toJson() // Pastikan ProductModel punya toJson
//             })
//         .toList();

//     await prefs.setString('guest_cart', json.encode(listMap));
//   }
...
//     _optimisticQty.remove(cartId);
//     setState(() {
//       _currentItems.removeWhere((item) => item.id == cartId);
...
//       _saveGuestCart();
//     } else {
//       context.read<CartBloc>().add(DeleteCartItemEvent(cartId));
//     }
//   }
...
//         ),
//         body: _isGuest
//             ? _buildBody(context, currencyFormat)
//             : _buildAuthBody(context, currencyFormat),
//       ),
//     );
//   }

//   // Builder khusus untuk pengguna yang sudah login
//   Widget _buildAuthBody(BuildContext context, NumberFormat currencyFormat) {
...
//             _currentSummary = state.summary;
//             final serverIds = state.items.map((e) => e.id).toSet();
//             _optimisticQty.removeWhere((id, optVal) {
//               if (!serverIds.contains(id)) return true;
//               return state.items.firstWhere((e) => e.id == id).quantity ==
//                   optVal;
//             });
//           });
//         }
...
//               child: CircularProgressIndicator(color: Colors.black));
//         }
//         return _buildBody(context, currencyFormat);
//       },
//     );
//   }

//   // Konten UI yang sama untuk Guest maupun Member
//   Widget _buildBody(BuildContext context, NumberFormat currencyFormat) {
//     if (_currentItems.isEmpty) return _buildEmptyCart(context);
...
//                       TextButton(
//                         onPressed: () {
//                           // Karena _deleteCartItem mengubah state list _currentItems, kita copy id dulu
//                           List<int> toDelete = List.from(_selectedIds);
...
//                       ? item.product!.discountPrice!
//                       : item.product!.price;
//                   int currentQty = _optimisticQty[item.id] ?? item.quantity;
...
//                         Checkbox(
//                           value: _selectedIds.contains(item.id),
//                           onChanged: (val) => _toggleItem(val, item.id, items),
//                           activeColor: Colors.black,
//                         ),
//                         GestureDetector(
//                           onTap: () => Navigator.push(
...
//                     //     ? null
//                     //     : () {
//                     //         // Siapkan Data Guest (List of map produk) untuk Payload Checkout nanti
//                     //         List<Map<String, dynamic>> guestPayload = [];
//                     //         if (_isGuest) {
//                     //           final selectedItems = _currentItems
//                     //               .where((e) => _selectedIds.contains(e.id))
//                     //               .toList();
//                     //           guestPayload = selectedItems
//                     //               .map((e) => {
//                     //                     'product_id': e.product!.id,
//                     //                     'quantity':
//                     //                         _optimisticQty[e.id] ?? e.quantity,
//                     //                     'color': e.color,
//                     //                   })
//                     //               .toList();
//                     //         }
...
//                                             _optimisticQty[e.id] ?? e.quantity,
//                                         'color': e.color,
//                                         // 👇 TAMBAHKAN INI AGAR PAYMENT PAGE BISA BACA FOTO & HARGA 👇
//                                         'product': {
//                                           'id': e.product!.id,
//                                           'name': e.product!.name,
...
class _CartPageState extends State<CartPage> {
  ...
  // 👇 PERBAIKAN 1: Wajib menyertakan cek hasActiveDiscount 👇
  num _calculateSaved(List<CartModel> items) {
    num saved = 0;
    for (var item in items) {
      if (_selectedIds.contains(item.id) && item.product != null) {
        ...
        // Sekarang sistem mengecek apakah diskon benar-benar masih aktif
            ...
            child: Column(
              ...
              children: [
                ...
                ...items.map((item) {
                  ...
                  // 👇 PERBAIKAN PADA TAMPILAN: CEK hasActiveDiscount 👇
                  bool hasDiscount = item.product!.hasActiveDiscount;
                  num activePrice = hasDiscount
                      ? item.product!.discountPrice!
                      : item.product!.price;
                              ...
                              guestPayload = selectedItems
                                  .map((e) => {
                                        ...
                                        'quantity':
                                            _optimisticQty[e.id] ?? e.quantity,
                                        'color': e.color,
                                        // 👇 PERBAIKAN 2: SUNTIKKAN TANGGAL DISKON AGAR PAYMENT PAGE BISA VALIDASI 👇
                                        'product': {
                                          'id': e.product!.id,
                                          'name': e.product!.name,
                                          'image': e.product!.image,
