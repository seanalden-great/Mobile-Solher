...
class _ExplorePageState extends State<ExplorePage> {
  ...
  // 👇 [BARU] State untuk menyimpan status toggle Final Sale 👇
  bool _isFinalSaleFilter = false;
      ...
      child: Scaffold(
        ...
        body: Column(
          ...
          children: [
            ...
            // 👇 [BARU] Sisipkan widget Filter Tambahan di sini 👇
            _buildExtraFilters(),
  ...
  // 👇 [BARU] Widget Filter Tambahan (Final Sale) 👇
  Widget _buildExtraFilters() {
    return Padding(
      ...
      child: Row(
        children: [
          FilterChip(
            label: const Text('FINAL SALE'),
            selected: _isFinalSaleFilter,
            onSelected: (bool selected) {
              setState(() {
                _isFinalSaleFilter = selected;
              });
            },
            showCheckmark: false,
            ...
            shape: StadiumBorder(
              side: BorderSide(
                color: _isFinalSaleFilter
                    ? Colors.red.shade700
                    : Colors.grey.shade300,
              ),
            ),
            labelStyle: TextStyle(
              color: _isFinalSaleFilter ? Colors.white : Colors.black87,
              fontSize: 10,
              fontWeight: FontWeight.bold,
              letterSpacing: 1.2,
            ),
    ...
    return BlocBuilder<ProductBloc, ProductState>(
      builder: (context, state) {
        if (state is ProductLoading) {
          ...
          // 👇 [DIUBAH] Aplikasikan filter Final Sale di List lokal jika toggle aktif 👇
          List<ProductModel> products = state.products;

          if (_isFinalSaleFilter) {
            products = products.where((p) => p.isFinalSale).toList();
          }
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                      _isFinalSaleFilter
                          ? "Tidak ada produk Final Sale di kategori ini."
