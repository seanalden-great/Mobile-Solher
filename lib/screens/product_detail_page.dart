...
//     // 👇 [PERBAIKAN] Gunakan fallback ke initialProduct jika API detail mengembalikan 0
//     int displayTotalSold = product.totalSold > 0
//         ? product.totalSold
//         : widget.initialProduct.totalSold;
...
//         crossAxisAlignment: CrossAxisAlignment.start,
//         children: [
//           // 👇 [PERBAIKAN] Gunakan variabel displayTotalSold
//           if (displayTotalSold > 0) ...[
//             Row(
//               children: [
//                 const Icon(Icons.check_circle, color: Colors.green, size: 16),
//                 const SizedBox(width: 6),
//                 Text('Terjual $displayTotalSold+',
//                     style: TextStyle(
//                         fontSize: 12,
//                         fontWeight: FontWeight.bold,
//                         color: Colors.grey.shade700)),
...
import 'dart:math';
import 'dart:convert'; // 👇 [BARU] Ditambahkan untuk parsing JSON lokal Guest Cart
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:intl/intl.dart';
import 'package:share_plus/share_plus.dart';
import 'package:shared_preferences/shared_preferences.dart'; // 👇 [BARU] Import SharedPrefs
...
class _ProductDetailPageState extends State<ProductDetailPage> {
  ...
  // 👇 [FUNGSI HELPER] Pencegah Error Mappings JSON 👇
  ...
  // 👇 [FUNGSI BARU] Aksi Add To Cart Cerdas (Guest & Auth) 👇
  Future<void> _handleAddToCart(bool isGuest, ProductModel product) async {
    final color = _selectedColor.isEmpty
        ? _extractColorName(product.name)
        : _selectedColor;
    ...
    if (isGuest) {
      ...
      List<dynamic> guestCart = [];

      if (cartStr != null && cartStr.isNotEmpty) {
        guestCart = json.decode(cartStr);
      }

      int existingIndex = guestCart.indexWhere(
          (item) => item['product_id'] == product.id && item['color'] == color);

      if (existingIndex >= 0) {
        guestCart[existingIndex]['quantity'] += _quantity;
      } else {
        guestCart.add({
          ...
          'quantity': _quantity,
          'color': color,
          'product': _productToMap(product) // Aman dari error format json
        });
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Added to Bag (Guest)'),
          backgroundColor: Colors.green,
        ));
        Navigator.push(
            context, MaterialPageRoute(builder: (_) => const CartPage()));
  ...
  // 👇 [FUNGSI BARU] Aksi Buy Now Cerdas (Guest & Auth) 👇
  Future<void> _handleBuyNow(bool isGuest, ProductModel product) async {
    final color = _selectedColor.isEmpty
        ? _extractColorName(product.name)
        : _selectedColor;

    if (isGuest) {
      final dummyId = DateTime.now().millisecondsSinceEpoch;
      ...
      Navigator.push(
          ...
          MaterialPageRoute(
              builder: (_) => PaymentPage(
                    selectedCartIds: [dummyId],
                    guestCartItems: guestPayload,
                  )));
    } else {
      context.read<CartBloc>().add(BuyNowEvent(
            productId: product.id,
  ...
  // ... Biarkan _buildRecommendations, _buildSuggestedProductCard, dll SAMA PERSIS SEPERTI SEBELUMNYA ...
                      ...
                      children: [
                        Expanded(
                          child: OutlinedButton(
                            ...
                            // 👇 PANGGIL FUNGSI CERDAS 👇
                            onPressed: () => _handleAddToCart(isGuest, product),
                            child: const Text('ADD TO BAG',
                                style: TextStyle(
                                    fontSize: 11,
                                    fontWeight: FontWeight.bold,
                        ...
                        Expanded(
                          child: ElevatedButton(
                            ...
                            // 👇 PANGGIL FUNGSI CERDAS 👇
                            onPressed: () => _handleBuyNow(isGuest, product),
                            child: BlocBuilder<CartBloc, CartState>(
                                builder: (context, state) {
                              if (state is CartLoading && !isGuest) {
                                return const SizedBox(
                                    height: 16,
                                    width: 16,
                                    child: CircularProgressIndicator(
